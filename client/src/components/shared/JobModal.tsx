    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='max-w-sm max-h-11/12 overflow-scroll no-scrollbar'>
        <DialogDescription>
          ...
          <Button onClick={() => console.log(form.state)}>asdasdas</Button>
        </DialogDescription>
        <DialogHeader>
                ...
                {({ state, handleChange }) => (
                  <div className='grid gap-3'>
                    ...
                    <DateTimePicker
                      showLabel={false}
                      onChange={(val) => {
                        console.log('🚀 ~ JobModal ~ val:', val);
                        if (!val.date || !val.time) return;

                        const [hours, minutes, seconds] = val.time
                          .split(':')
                          .map(Number);
                        const combinedDate = new Date(val.date);
                        combinedDate.setHours(hours, minutes, seconds, 0);
                        console.log(
                          '🚀 ~ JobModal ~ combinedDate:',
                          combinedDate
                        );

                        handleChange(combinedDate.toISOString());
                      }}
                      value={{ date: state.value, time: state.value }}
                    />
                  </div>
                )}
              </form.Field>
                ...
                {({ state, handleChange }) => (
                  <div className='grid gap-3'>
                    <Label htmlFor='interview-type'>
                      Location {JSON.stringify(state.value)}
                    </Label>
                    <DropdownMenu>
                      <DropdownMenuTrigger className='border border-input rounded-md h-8'>
                        <p className='text-sm'>
                          {state.value
                            ? InterviewTypeLabels[state.value as InterviewType]
                            : 'Select Type'}
                        </p>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent>
